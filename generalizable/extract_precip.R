...
TARGET_CACHE_DIR <- "prism_ca_hucs"   # cropped regional tiles go here
...
prism_index <- data.table(file = prism_files, date = prism_dates) %>%
  filter(!is.na(date)) %>%
  arrange(date)
prism_index <- prism_index[date >= as.Date("2009-01-01")]
prism_index <- data.table(file = prism_files, date = prism_dates) %>%
...
to_process <- prism_index[
  !sprintf("prism_ppt_us_25m_%s_sac.tif",
...
# =============================================================================
# Output: prism_ca_hucs/PRISM_ppt_4kmD2_YYYYMMDD_sac.tif
...
plan(sequential)
gc()
plan(multisession, workers = 4)  # start conservative
crop_one <- function(i, files, dates, target_bbox_vec, TARGET_CACHE_DIR, p) {
  ...
  out_file <- file.path(TARGET_CACHE_DIR,
                        sprintf("prism_ppt_us_25m_%s_sac.tif", date_str))
...
PRECIP_HUC12_DIR <- "data/precip_huc12_tiles_ca"
dir.create(PRECIP_HUC12_DIR, showWarnings = FALSE, recursive = TRUE)

already_done <- list.files(PRECIP_HUC12_DIR, pattern = "\\.fst$")

process_year <- function(yr, tif_dt, huc12_target, out_dir, already_done) {
  yr     <- as.integer(yr)
  yr_rows <- tif_dt[tif_dt$yr_int == yr, ]
  
  huc12_todo_idx <- which(
    !sprintf("precip_%s_%d.fst", huc12_target$huc12, yr) %in% already_done
  )
  ...
  # Reproject HUC12s to match raster CRS
  huc12_proj <- sf::st_transform(huc12_target, crs = terra::crs(r_yr))
...
with_progress({
  ...
  future_lapply(years, function(yr) {
    on.exit(p())
    process_year(yr, tif_dt, huc12_target, PRECIP_HUC12_DIR, already_done)
  }, future.seed = TRUE)
})
...
saveRDS(precip_all, "sac/data/precip_huc12s_sac.rds")
