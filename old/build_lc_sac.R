...
# --------------------------------------------------
# Add undeveloped to the SAME lc_shares dataframe
# --------------------------------------------------
for (yr in years) {
...
# Pivot ALL land cover variables, including undeveloped
lc_long <- lc_shares %>%
  pivot_longer(
    cols = -huc12,
...
# Area-weighted upstream land cover per tract-year
...
# Unweighted upstream land cover per tract-year
...
# Save both
saveRDS(tract_year_lc_areaweighted, "sac/data/tract_year_lc_aw.rds")
saveRDS(tract_year_lc_unweighted, "sac/data/tract_year_lc.rds")
